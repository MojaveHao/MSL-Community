...
namespace MSL.controls
{
    internal class Functions
    {
        ...
        {
            if (version.StartsWith("*")) return Tuple.Create(100, 100, 100, 100);
            // 使用正则表达式从版本号中提取主要版本号
            ...
            foreach (string part in versionParts) { if (int.TryParse(part, out int parsedPart)) versionIntParts.Add(parsedPart); }

            // 添加0，以便对不完整的版本号进行比较（如1.7）
            while (versionIntParts.Count < 4) { versionIntParts.Add(0); }
        ...
        public static void MoveFolder(string sourcePath, string destPath)
        {
            if (Directory.Exists(sourcePath))
            {
                ...
                {
                    //目标目录不存在则创建
                    try { _ = Directory.CreateDirectory(destPath); }
                    catch (Exception ex) { throw new Exception("创建目标目录失败：" + ex.Message); }
                }
                //获得源文件下所有文件
            ...
            else throw new DirectoryNotFoundException("源目录不存在");
        }
    }
}
